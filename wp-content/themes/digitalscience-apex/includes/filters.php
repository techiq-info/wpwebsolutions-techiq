...
function apex_gallery_slider_top( $post_id, $meta_data ) {
	...
	if( $directional_nav ) {
		$html = '<div class="mtphr-gallery-header">';
			$html .= '<a href="#" class="mtphr-gallery-nav-prev" rel="nofollow"><i class="apex-icon-arrow-up"></i></a>';
			if( !is_single() && get_post_format() != 'gallery' ) {
				$html .= '<a href="#" class="mtphr-gallery-close" rel="nofollow"><i class="apex-icon-close-1"></i></a>';
			}
			$html .= '<a href="#" class="mtphr-gallery-nav-next" rel="nofollow"><i class="apex-icon-arrow-down"></i></a>';
		$html .= '</div>';
		echo $html;
	}
}
...
function apex_gallery_post_block( $block, $excerpt, $args, $permalink=false ) {
	...
	$html .= '<div class="entry-header jquery-parallax" style="background:url('. get_the_post_thumbnail_url( get_the_id() ).') 50% 0 no-repeat fixed; background-size:cover" >
				
		';
			
		$html .= '<div class="entry-featured-archive" style="min-height:280px;">';
		#style="background-image:url(\''.get_the_post_thumbnail_url( get_the_id() ).'\'); background-size:cover; background-position:center; background-repeat:no-repeat; width:100%; height:280px"
			#$html .= get_mtphr_gallery_thumbnail( get_the_id() );
			#$html .='<div class="parallax-container" data-parallax="scroll" data-position="top" data-bleed="10" data-image-src="'. get_the_post_thumbnail_url( get_the_id() )  .'"  data-natural-height="540" style="min-height:570px;"></div>';
