...
fabric.Pattern = fabric.util.createClass(/** @lends fabric.Pattern.prototype */ {
  ...
  toLive: function(ctx) {
	...
	//console.log(source._element);
	//MODIF GARREL
   // return ctx.createPattern(source._element, this.repeat);
...
(function () {
  ...
  fabric.StaticCanvas = fabric.util.createClass(/** @lends fabric.StaticCanvas.prototype */ {
    ...
    _shouldRenderObject: function(object) {
      ...
      return (true || !this.preserveObjectStacking);
    },
    ...
    renderAll: function (allOnTop) {
      var canvasToDrawOn = this[(allOnTop === true && this.interactive) ? 'contextTop' : 'contextContainer'];
			...
			//console.log("Render All "+canvasToDrawOn);
    ...
    _toObjectMethod: function (methodName, propertiesToInclude) {
      ...
      extend(data, this.__serializeBgOverlay());
		extend(data, this.__getImageSpace());
     ...
     * @chainable
     */
    sendToBack: function (object,render) {
      removeFromArray(this._objects, object);
      this._objects.unshift(object);
      if (render) return this.renderAll && this.renderAll();
    },
     ...
     * @chainable
     */
    bringToFront: function (object,render) {
      removeFromArray(this._objects, object);
...
(function() {
  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, /** @lends fabric.Canvas.prototype */ {
    ...
    _getOriginFromCorner: function(target, corner) {
      ...
      if (corner === 'ml' || corner === 'tl' || corner === 'bl' || corner === 'ml2' || corner === 'tl2' || corner === 'bl2') {
        origin.x = 'right';
      }
      else if (corner === 'mr' || corner === 'tr' || corner === 'br'|| corner === 'mr2' || corner === 'tr2' || corner === 'br2') {
        origin.x = 'left';
      }

      if (corner === 'tl' || corner === 'mt' || corner === 'tr' || corner === 'tl2' || corner === 'mt2' || corner === 'tr2') {
        origin.y = 'bottom';
      }
      else if (corner === 'bl' || corner === 'mb' || corner === 'br' || corner === 'bl2' || corner === 'mb2' || corner === 'br2') {
        origin.y = 'top';
      }
    ...
    _getActionFromCorner: function(target, corner) {
      var action = 'drag';
      if (corner) {
        action = (corner === 'ml' || corner === 'mr' ||corner === 'ml2' || corner === 'mr2'  )
          ? 'scaleX'
          : (corner === 'mt' || corner === 'mb' || corner === 'mt2' || corner === 'mb2')
            ? 'scaleY'
            : (corner === 'tr' || corner === 'bl' || corner === 'tr2' || corner === 'bl2')
			? 'rotate'			
            : (corner === 'mtr' || corner === 'mtr2')
			// ? 'rotate'
			 ? 'pattern'
              : 'scale';
      }
    ...
    _translateObject: function (x, y) {
      var target = this._currentTransform.target;
		var is_move_x = is_move_y = true;
		var i =0;
		
		
		
		if (x!=this._currentTransform.ex || y!=this._currentTransform.ey) {
		
			//Grille magnetique
			var newcenterx = this._currentTransform.centerX + (x-this._currentTransform.ex);
			var newcentery = this._currentTransform.centerY + (y-this._currentTransform.ey);
			
			
			
			var is_magnetic_x = is_magnetic_y = false;
			
			for (i=0 ; i < left_magnetic.length; i++) {		
				
				if (newcenterx < (left_magnetic[i] +val_magnetisme) && newcenterx > (left_magnetic[i] -val_magnetisme)) {
					x=x-(newcenterx-left_magnetic[i]);;
					is_magnetic_x = true;
				}
				
				if (newcentery < (top_magnetic[i] +val_magnetisme) && newcentery > (top_magnetic[i] -val_magnetisme)) {
					is_magnetic_y = true;
					y=y-(newcentery-top_magnetic[i]);;
				}
			}
			
			if (is_magnetic_x || 
				(((this._currentTransform.ex - x) > 0 && (this._currentTransform.ex - x)<this._currentTransform.centerX) || 
					((x-this._currentTransform.ex ) > 0 && (x-this._currentTransform.ex )<(CraPacrAxx.getWidth()-this._currentTransform.centerX)))){
						
						if (!is_changing && this._currentTransform.left!=target.left) {
							is_changing=true;
							save();
						  }
						
						if (target.type=='imagepattern' && target.angle==0) {
							if ((x - this._currentTransform.offsetX) < 0) target.set('left',0);
							else if ((x - this._currentTransform.offsetX) > (CraPacrAxx.getWidth() - target.getWidth())) target.set('left',(CraPacrAxx.getWidth() - target.getWidth()));
							else target.set('left',x - this._currentTransform.offsetX);
						} else target.set('left',x - this._currentTransform.offsetX);
			}
					
			if (is_magnetic_y || (((this._currentTransform.ey - y) > 0 && (this._currentTransform.ey - y)<this._currentTransform.centerY) || ((y-this._currentTransform.ey ) > 0 && (y-this._currentTransform.ey )<(CraPacrAxx.getHeight()-this._currentTransform.centerY)))){
						if (!is_changing   && this._currentTransform.top!=target.top) {
							is_changing=true;
							save();
						  }
						
						if (target.type=='imagepattern' && target.angle==0) {
							if ((y - this._currentTransform.offsetY) < 0) target.set('top',0);
							else if ((y - this._currentTransform.offsetY) > (CraPacrAxx.getHeight() - target.getHeight())) target.set('top',(CraPacrAxx.getHeight() - target.getHeight()));
							else target.set('top',y - this._currentTransform.offsetY);
						} else target.set('top',y - this._currentTransform.offsetY);
			} 
		}
    },
     ...
     * @param {Number} y pointer's y coordinate
     */
    _translatePattern : function (x, y) {
		
		
		var target = this._currentTransform.target;			
		target._transPattern(x,y);
    ...
    _scaleObject: function (x, y, by) {
      ...
      this._setLocalMouse(localMouse, t);
		  ...
		  if (target.type=='imagepattern' && target.angle===0 ) {
			 if (by=='equally') { 				
				this._setObjectPattern(localMouse, t, 'x',x,y);			
				this._setObjectPattern(localMouse, t, 'y',x,y);			
			} else this._setObjectPattern(localMouse, t, by,x,y);		
    ...
    _setObjectPattern: function(localMouse, transform, by,x,y) {
		
		var target = this._currentTransform.target;
		var original = this._currentTransform.original;
		
		
		var rect_x = target.getWidth();
		var rect_y = target.getHeight();
		
		var new_w,new_h,new_l,new_t,new_o_w;
		var nochange=false;
		
		if (target.angle==0) {
			if (by === 'x') {
				
				if (x > CraPacrAxx.getWidth()) {
					if ( target.left < 1 ) nochange = true;
					
					x = (CraPacrAxx.getWidth()+((x - CraPacrAxx.getWidth())/9));				
				} else if (x < 0) {
					x = (x /9);
					if ((-x + target.left + target.getWidth())>CraPacrAxx.getWidth()) nochange = true;
					
				}
				
				if (!nochange) {
					if (original.originX=='left') {			
					
						new_w = x - target.left;
						
						if (new_w > 20) {
							if ((new_w + target.left) >  CraPacrAxx.getWidth()) {					
								new_l = (CraPacrAxx.getWidth()-new_w);
								if (new_l<0) new_l=0;
							} 
						
							if (new_w < CraPacrAxx.getWidth()) {
								target.set('width',new_w);
								if (new_l!=undefined) target.set('left', new_l);
							} else {
								new_w = CraPacrAxx.getWidth();
								if (new_l!=undefined) target.set('left', new_l);
							}
						}	
					} else {
										
						new_w = target.getWidth() + (target.left - x);
						...
						target.set('width',CraPacrAxx.getWidth());
						target.set('left',0);
				}
			} else {
				
				
				if (y > CraPacrAxx.getHeight()) {				
					if ( target.top < 1 ) nochange = true;
					
					x = (CraPacrAxx.getHeight()+((y - CraPacrAxx.getHeight())/9));	
					
				} else if (y < 0) {
					if ((-y + target.top + target.getHeight())>CraPacrAxx.getHeight()) nochange = true;
					y = (y/9);
				}
				
				
				if (!nochange) {
					if (original.originY=='top') {			
					
						new_h = y - target.top;
						
						if (new_h > 20) {
						
							if ((new_h + target.top) >  CraPacrAxx.getHeight()) {					
								new_t = (CraPacrAxx.getHeight()-new_h);
								if (new_t<0) new_t=0;
							} 
						
							if (new_h < CraPacrAxx.getHeight()) {
								target.set('height',new_h);
								if (new_t!=undefined) target.set('top', new_t);
							} else {
								new_h = CraPacrAxx.getHeight();
								if (new_t!=undefined) target.set('top', new_t);
							}
						}	
					} else {
										
						new_h = target.getHeight() + (target.top - y);
						new_t=y;
						if (new_h > 20) {
							if ( y < 0) {					
								new_t=0;
						...
						target.set('height',CraPacrAxx.getHeight());
						target.set('top',0);
				}
    ...
    _setObjectScale: function(localMouse, transform, lockScalingX, lockScalingY, by, lockScalingFlip) {
	 ...
	 // console.log( by);
      ...
      if ( transform.newScaleX <= 0.05 ) {
        forbidScalingX = true;
      }

      if ( transform.newScaleY <= 0.05) {
        forbidScalingY = true;
      }
      ...
      else if (by === 'x' && !target.get('lockUniScaling') && !forbidScalingX && !lockScalingX ) {
		  
		if ( target.type=='imagepattern') {
				target.set('width', transform.newScaleX*target.width);
				target._scalePattern();
		} else target.set('scaleX', transform.newScaleX);		
      }
      else if (by === 'y' && !target.get('lockUniScaling') && !forbidScalingY && !lockScalingY ) {
       // forbidScalingY || lockScalingY || target.set('scaleY', transform.newScaleY);
		
		if ( target.type=='imagepattern') {
				target.set('height', transform.newScaleY*target.height);
				target._scalePattern();
		} else target.set('scaleY', transform.newScaleY);	
      }
...
//console.log(target.type);
		 if ( target.type=='imagepattern') {
			...
			target.set('height', transform.newScaleY*target.height);
			 ...
			 var ftSiz = target.fontSize + (target.fontSize * (transform.newScaleX - transform.original.scaleX));
                if (ftSiz > 255) {
                    ftSiz = 255
                }
                if (ftSiz < 10) {
    ...
    _rotateObject: function (x, y) {
	  ...
	  angle = angle % 360;
		//angle = angle % 360;
      if (angle > 357 || angle < 3) angle=0;
	  else if (angle > 87 && angle < 93) angle =90;
	  else if (angle > 177 && angle < 183) angle =180;
		else if (angle > 267 && angle < 273) angle =270;
 ...
 removeListener(this.upperCanvasEl, 'mousewheel', this._onMouseWheel);
    ...
    _onDrag: function(e, self) {
     ...
     _onMouseWheel: function(e, self) {
		...
		var obj = CraPacrAxx.getActiveObject();
			if (obj && obj.type=='imagepattern') {	
				
				//console.log(obj.oCoords);
				
				var delta = e.wheelDelta / 1200;
				
				var x = obj.get('scaleImg')-delta;
				
				if (x <0.3)x = 0.3;
				if (x>1)x=1;
				//console.log(x);
				//change_value_object("slider-taille-zoom",x);
				$('#slider-taille-zoom').val(x).change();	
				//slider_taille.suBString(x, 0, true);
    ...
    __onMouseDown: function (e) {
      ...
      if (target && target.selectable ) {
		...
		if (this._currentTransform.action=='pattern') {
			var pointer = this.getPointer(e);
			this._currentTransform.target.set('leftInit',pointer.x);
			this._currentTransform.target.set('topInit',pointer.y);
		}
	  }
	...
	/*	console.log(transform);
	  ...
	  if (action === 'pattern') {
		//  console.log('pattern');
        this._translatePattern(x, y);
        this._fire('pattern', target, e);
		...
		if (target.isbackground!=true){	
			...
			//if (target.type=='imagepattern') target.ismove = true;
    ...
    _setCornerCursor: function(corner, target) {
      ...
      else if ((corner === 'mtr' || corner === 'mtr2') && target.hasRotatingPoint) {
...
fabric.util.object.extend(fabric.StaticCanvas.prototype, /** @lends fabric.StaticCanvas.prototype */ {
  ...
  _setBgOverlay: function(serialized, callback) {
	...
	this['imageSpace'] = serialized.imageSpace;  
	...
	//this.__setBgOverlay('imageSpace', serialized.imageSpace, loaded, cbIfLoaded);
    cbIfLoaded();
  },
...
//	ltPoint:null,
    ...
    transform: function(ctx, fromLeft) {
	  ...
	  this.centerPoint = center.x + "," + center.y + "," + degreesToRadians(this.angle) + "," + this.scaleX * (this.flipX ? -1 : 1) + "," + this.scaleY * (this.flipY ? -1 : 1);
    },
    ...
    _setStrokeStyles: function(ctx) {
      if (this.stroke) {
        if ( this.strokeWidth!=0) ctx.lineWidth = this.strokeWidth;
		else ctx.lineWidth = this.borderSize;
    ...
    _renderControls: function(ctx, noTransform) {
      ...
      ctx.translate(center.x, center.y);
      ctx.rotate(degreesToRadians(this.angle));
	  if (!this.isbackground) this.drawBorders(ctx);
    ...
    _renderStroke: function(ctx) {
      if (!this.stroke || (this.strokeWidth === 0 && (!this.borderSize || this.borderSize === 0) ) ) {
        return;
      } 
  ...
  fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
    ...
    setCoords: function() {
      ...
      var _hypotenuse = Math.sqrt(
		  ...
		  mtr = new fabric.Point( (((((br.x - tr.x)*3)/4)+tr.x) + ((((bl.x - tl.x)*3)/4)+tl.x))/2 ,(((((br.y - tr.y)*3)/4)+tr.y) + ((((bl.y - tl.y)*3)/4)+tl.y))/2);
			
		  var width_tot = CraPacrAxx.getWidth();	
		  var height_tot = CraPacrAxx.getHeight();
			
		  if (this.type=='imagepattern') {
		 ...
		 this.oCoords = {
			...
			var mid = this.cornerSize/2;
		 for (var i in this.oCoords) {		 
			 
			if (this.oCoords[i].x <mid) l_temp = mid;
			else if (this.oCoords[i].x>(width_tot-mid)) l_temp = width_tot-mid;
			else l_temp = this.oCoords[i].x;
			if (this.oCoords[i].y <mid) t_temp = mid;
			else if (this.oCoords[i].y>(height_tot-mid)) t_temp = height_tot-mid;
			else t_temp = this.oCoords[i].y;  
			temp = new fabric.Point(l_temp,t_temp);
			this.oCoords[i+"2"]  = temp;
...
fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
  ...
  sendToBack: function() {
      ...
      this.canvas.sendToBack(this,true);
  ...
  bringToFront: function() {
      ...
      this.canvas.bringToFront(this,true);
  ...
  fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
    ...
    _findTargetCorner: function(pointer) {
      ...
      for (var i in this.oCoords) {
        ...
        if ( this.type==='text' &&
           (i === 'mt' || i === 'mr' || i === 'mb' || i === 'ml' || i === 'mt2' || i === 'mr2' || i === 'mb2' || i === 'ml2')) {
          continue;
        }
	...
	if (!this.isbackground) {
      ...
      // top-left
      this._drawControl('tl', ctx, "resize_angle",
        left - scaleOffset,
        top - scaleOffset);

      // top-right
      this._drawControl('tr', ctx, "rotate",
        left + width - scaleOffset,
        top - scaleOffset);

      // bottom-left
      this._drawControl('bl', ctx, "rotate",
        left - scaleOffset,
        top + height - scaleOffset);

      // bottom-right
      this._drawControl('br', ctx,  "resize_angle",
        left + width - scaleOffset,
        top + height - scaleOffset);
      ...
      if (this.type!='text' ) {
        ...
        // middle-top
        this._drawControl('mt', ctx,  "resize_horizontal",
          left + width/2 - scaleOffset,
          top - scaleOffset);

        // middle-bottom
        this._drawControl('mb', ctx, "resize_horizontal",
          left + width/2 - scaleOffset,
          top + height - scaleOffset);

        // middle-right
        this._drawControl('mr', ctx,  "resize_vertical",
          left + width - scaleOffset,
          top + height/2 - scaleOffset);

        // middle-left
        this._drawControl('ml', ctx,  "resize_vertical",
          left - scaleOffset,
          top + height/2 - scaleOffset);
      }
	}
	...
	if (this.type=='imagepattern') {
      ...
      if (this.hasRotatingPoint) {
        this._drawControl('mtr', ctx, "deplace",
          left + width/2- this.cornerSize ,
          top + ((3*height)/4) - this.cornerSize );
      }
	}
      ctx.restore();
	 this.setCoords();
    ...
    _drawControl: function(control, ctx, methodName, left, top) {
	  ...
	  if ('mtr'==control) var size = this.cornerSize*2;
      else var size = this.cornerSize;
	  var mid = this.cornerSize/2;
     // isVML() || this.transparentCorners || ctx.clearRect(left, top, size, size);
		
		 
		if (this.oCoords[control].x != this.oCoords[control+'2'].x ||  this.oCoords[control].y != this.oCoords[control+'2'].y  ) {
			
			var new_left=new_top=0;
			
			new_left = (this.oCoords[control+'2'].x-this.oCoords[control].x-mid);
			new_top = (this.oCoords[control+'2'].y-this.oCoords[control].y-mid);
			
			ctx.save();
			
			ctx.translate(left+mid,top+mid);
			ctx.rotate((360-this.angle)*Math.PI/180);
			
			var source = new Image();
			source.src = IMAGE_INTERF+methodName+'.svg';
			source.width = '25';
			source.height = '25';
						
			ctx.drawImage(source,new_left,new_top);
			ctx.restore();
  ...
  fabric.Path = fabric.util.createClass(fabric.Object, /** @lends fabric.Path.prototype */ {
    ...
    _render: function(ctx) {
      ...
      ctx.beginPath();
		...
		//this.ltPoint=this.width/2+this.pathOffset.x+","+(this.height/2+this.pathOffset.y);
...
(function(global) {
  ...
  if (global.fabric.Imagepattern) {
    fabric.warn('fabric.Imagepattern is already defined.');
    return;
  }

   fabric.Imagepattern = fabric.util.createClass(fabric.Object, /** @lends fabric.Imagepattern.prototype */ {
	...
	toBackground:function(){
		...
		if (this.isbackground){		
			...
			CraPacrAxx.bringToFront(this,false);
			this._changeAndAnimateObject('w',this.width);	
		} else {
			var item = CraPacrAxx.getObjects();	
			if (item) {			
				for(var i=0;i<item.length;i++){
					if (item[i].isbackground) {
						item[i].left=10;
						item[i].top=10;
						item[i].isbackground=false;
						item[i].width=(item[i].width/3);
						item[i].height=(item[i].height/3);
						CraPacrAxx.bringToFront(item[i],false);		
						item[i].hoverCursor='move';							
						item[i]._changeAndAnimateObject('w',item[i].width);	
			...
			this.top = 0;
			this.angle=0;
			this.width = CraPacrAxx.getWidth();
			this.height = CraPacrAxx.getHeight();
			CraPacrAxx.sendToBack(this,false);			
    ...
    _stroke: function(ctx) {
      ...
      this._setStrokeStyles(ctx);
      ctx.beginPath();
      if (this.type=='imagepattern' && this._element!=null){
		  ctx.strokeRect((-this.width / 2)+(CraPacrAxx.imageSpace)+(this.borderSize / 2), (-this.height / 2)+(CraPacrAxx.imageSpace)+(this.borderSize / 2)
		  , this.width-(CraPacrAxx.imageSpace*2)-(this.borderSize), this.height-(CraPacrAxx.imageSpace*2)-(this.borderSize));
		...
		if (is_finish) {
		  ...
		  return extend(this.callSuper('toObject', propertiesToInclude), {
			...
			crossOrigin: this.crossOrigin,
			 ...
			 widthPat: this.widthPat,
			 heightPat: this.heightPat,
			 topPat: this.topPat,
			 leftPat: this.leftPat,
			 scalePat: this.scalePat,
			  ismove: this.ismove,
			   masque: this.masque,
    ...
    toString: function() {
      return '#<fabric.Imagepattern: { src: "' + this.getSrc() + '" }>';
    },
	...
	zoomObject: function(oldzoom){
		
		var change_t =  (this.heightPat*oldzoom )-(this.heightPat*this.scaleImg );
		var change_l =  (this.widthPat*oldzoom )-(this.widthPat*this.scaleImg );
		this.topPat = this.topPat+(change_t/2);
		this.leftPat = this.leftPat+(change_l/2);
		
		if (this.leftPat<0) this.leftPat = 0;
		if ((this.leftPat + (this.widthPat*this.scaleImg ))> this.widthImg) this.leftPat = this.widthImg - (this.widthPat*this.scaleImg );
		if (this.topPat<0) this.topPat = 0;
		if ((this.topPat + (this.heightPat*this.scaleImg ))> this.heightImg) this.topPat = this.heightImg - (this.heightPat*this.scaleImg );
		
	},

    _render: function(ctx, noTransform) {
		  var x, y, imageMargins = this._findMargins(), elementToDraw;
	//imagePlace = this._setImgInBox(),
		  x = (noTransform ? this.left : -this.width / 2);
		  y = (noTransform ? this.top : -this.height / 2);
		  ...
		  /*if (this.meetOrSlice === 'slice') {
		   ...
		   var padding = CraPacrAxx.imageSpace;
		   
			if (this.isbackground) padding=0;
		if (this.masque!='') {

			var clipZone = this._renderClip(this.masque,this.width-(padding*2)-this.borderSize,this.height-(padding*2)-this.borderSize);
		  
			ctx.save();
			ctx.moveTo((-1*(this.width))+padding+this.borderSize,(-1*(this.height))+padding+this.borderSize);
			this.masquesvg = clipZone.toSVG();
			//console.log(this.masquesvg);
			clipZone._render(ctx);
			this.path = clipZone.path;
		//	this.ltPoint = clipZone.ltPoint;
			ctx.clip(); 
		}	
		...
		if ( elementToDraw!=null) {
			...
			var wid_draw = 	Math.floor(this.width/2);
			var hei_draw = 	Math.floor(this.height/2);
			
			
			
			ctx.drawImage(elementToDraw, this.leftPat,this.topPat,
										 this.widthPat*this.scaleImg,
										this.heightPat*this.scaleImg,(-1*wid_draw)+padding,(-1*hei_draw)+padding,Math.floor(this.width)-(padding*2), Math.floor(this.height)-(padding*2)
										);
		...
		if (this.masque=='') this._renderStroke(ctx);
		
    },
     
	_renderClip: function (property,w,h) {
		var path_temp,scale_temp;
		if (property == "cercle") {
			path_temp = '<path d="M600,300c0,165.686-134.314,300-300,300C134.314,600,0,465.686,0,300C0,134.315,134.315,0,300,0 C465.686,0,600,134.314,600,300z"/>'
		}
		if (property == "coeur") {
			path_temp = '<path fill="#000000" d="M299.428,598c0,0-88.785-103.659-148.549-142.533C91.114,416.594,2.533,339.93,2.533,208.192 C2.533,76.457,94.314,0.871,178.627,4.11c84.312,3.24,120.801,76.666,120.801,76.666C318.435,45.142,354.721-4.729,455.04,4.01 c100.32,8.738,155.661,108.172,139.808,237.657C578.569,374.619,478.52,419.833,445.435,452.228 C412.35,484.623,328.448,546.171,299.428,598z"/>'
		}
		if (property == "coeur_2") {
			path_temp = '<path fill="#000000" d="M213.569,595.852c0,0-185.915-154.906-207.466-340.485C-15.447,69.788,102.939,2.204,143.091,3.97 s128.51-12.012,156.155,154.086c38.099-69.974,63.121-86.38,117.25-97.777c54.128-11.398,177.154,12.994,179.23,158.471 c2.076,145.477-35.271,182.299-129.404,229.223S351.989,472.056,213.569,595.852z"/>'
		}
		if (property == "etoile") {
			path_temp = '<polygon points="297.711,0 389.808,197.385 595.74,229.037 446.726,382.68 481.903,599.627 297.711,497.198 113.52,599.627 148.697,382.68 -0.317,229.037 205.615,197.385"/>'
		}
		if (property == "etoile_2") {
			path_temp = '<polygon points="299.051,-0.408 399.098,148.903 597.818,149.592 499.145,299.592 597.818,449.592 399.098,450.28 299.051,599.592 199.005,450.28 0.284,449.592 98.959,299.592 0.284,149.592 199.005,148.903"/>'
		}
		if (property == "etoile_3") {
			path_temp = '<polygon points="299.593,0 360.822,152.179 511.726,87.868 447.414,238.771 599.594,300 447.414,361.229 511.726,512.132 360.822,447.82 299.593,600 238.364,447.82 87.461,512.132 151.772,361.229 -0.407,300 151.772,238.771 87.461,87.868  238.364,152.179"/>'
		}
		if (property == "nuage") {
			path_temp = '<path fill="#000000" d="M53.31,494.385c0,0-47.635-28.03-46.612-71.397c0.485-20.588,11.937-44.633,44.658-70.775 c-17.39-25.907-25.551-60.441-15.863-88.388c7.78-22.442,27.07-40.637,62.333-46.703c0,0-31.072-74.587,7.838-118.039 c18.442-20.594,52.604-34.194,113.247-29.544C245.5,23.5,271.607,4.173,318.054,4.586s85.249,81.527,93.848,127.47 C430,112,475.273,116.586,499.637,147.293s25.783,77.944,24.323,87.076c23.184,10.448,58.302,28.784,65.921,71.958 s-16.865,82.998-16.865,82.998s21.65,12.176,21.567,47.176S576,518.5,537.5,513c-9,27.5-46.535,71.316-87.018,71.658 s-75.32-34.059-84.651-62.858c-8.331,25.2-41.166,68.864-97.249,70.782S189.1,538.41,182.8,512.955 c-9.8,25.545-53.356,66.223-87.328,55.134S54.62,517.27,53.31,494.385z"/>'
		}
		if (property == "square") {
			path_temp = '<path fill="#FF0000" d="M3,108.083C3,108.083,6.556,4,109.333,4S490,4,490,4s105,6.804,105,104.361S595,489,595,489 s-0.945,106-111.362,106S110,595,110,595S3,595.5,3,494.111S3,108.083,3,108.083z"/>'
		}
		if (property == "square_2") {
			path_temp = '<path fill="#FF0000" d="M3,37.778C3,37.778,5.167,4,42.667,4S566,4,566,4s29,2.056,29,33.528S595,563,595,563s-3,32-33,32 s-523,0-523,0s-36,2-36-37S3,37.778,3,37.778z"/>'
		}
		if (property == "fleur") {
			path_temp = '<path fill="#000000" d="M219.333,393.795c0,0-156.465,124.941-208.066,37.74s145.065-130.2,171.065-131.035 c-50.898-4.211-203.274-31.958-167.137-132.396s191.719,18.525,203.308,35.148C200.278,170.108,128.795,27.334,227.397,4.5 S326,167.666,324,182.332c9.308-32.218,64.449-173.157,159.558-121.245s-46.462,175.534-74.255,190.138 c51.339-14.389,180.918-35.539,189.474,46.783s-143.817,74.876-182.631,60.266c41.187,25.39,133.574,143.302,63.047,186.346 S328.528,457.274,320.491,412.95c8.037,54.657,15.497,184.78-87.165,184.08S199.335,417.927,219.333,393.795z"/>'
		}
		if (property == "etoile" || property == "etoile_2" || property == "etoile_3") {
			var p = path_temp.substring(path_temp.indexOf('points="') + 8, path_temp.indexOf('"/')).split(/\s+/);
			var path = "";
			for (var i = 0, len = p.length; i < len; i++) {
				path += (i && "L" || "M") + p[i]
			}
			path += "L" + p[0] + "L" + p[1];
			path_temp = '<path d="' + path + '"/>'
		}
		if (w != 600 || h != 600) {
			if (w != 600) {
				var raitow = w / 600
			...
			scale_temp = "scale(" + raitow + " " + raitoh + ")"
		} else {
			scale_temp = "scale(1 1)"
		}
		var path_def = get_substring(path_temp, scale_temp);
		path_def = path_def.substring(path_def.indexOf('d="'), path_def.indexOf('"/'));
		var poly = new fabric.Path(path_def);		
		
		if (this.borderSize!=0) poly.strokeWidth=this.borderSize;
		poly.stroke=this.stroke;
		//console.log(poly);
	 ...
	 _transPattern: function (x,y){
		 
	
		
		if ((x-this.leftInit )!=0) {
			this.leftPat=this.leftPat - ((x-this.leftInit)/2);
		...
		if ((y-this.topInit )!=0) {
			this.topPat = this.topPat - ((y-this.topInit)/2);
	...
	_scalePattern: function () {
		
		var savWidth=this.widthPat,savHeigh=this.heightPat;
		
		var isWidth = this._getSensImg();
		
		
		if (this.scaleImg==1){
			if (isWidth) { 			
				this.leftPat=0;
				this.topPat = this.topPat + ((savHeigh -  this.heightPat)/2); 
				if (this.topPat<0) this.topPat = 0;
				if ((this.topPat + this.heightPat)> this.heightImg) this.topPat = this.heightImg - this.heightPat;
							
			} else { 			
				this.leftPat=this.leftPat + ((savWidth -  this.widthPat)/2); 
				this.topPat = 0;
				if (this.leftPat<0) this.leftPat = 0;
				if ((this.leftPat + this.widthPat)> this.widthImg) this.leftPat = this.widthImg - this.widthPat;
	...
	_changeAndAnimateObject: function (init,val) {
		
		// BUG FIREFOX A VALIDER
		var is_firefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
		
		if (is_firefox) {
			if (init=='init') {			
				this._setImgInBoxDft();
				CraPacrAxx.add(this);
			} else {	
			...
			var sav_w = this.width;		
			var sav_h = this.height;
			...
			if (init=='init') {			
				...
				this.animate('width', sav_w, { 
				  duration: 150,
				  onChange: CraPacrAxx.renderAll.bind(CraPacrAxx),
				  easing: fabric.util.ease['easeOutCirc']
				});
				this.animate('height', sav_h, {
				  duration: 150,
				  onChange: CraPacrAxx.renderAll.bind(CraPacrAxx),
				  easing: fabric.util.ease['easeOutCirc']
				});
	 ...
	 _setImgInBoxDft: function() {
	
		var isWidth = this._getSensImg();
		
		if (isWidth) { 			
			this.leftPat=0;
			this.topPat = (this.heightImg-this.heightPat)/2;
						
		} else { 			
			this.leftPat=(this.widthImg-this.widthPat)/2;
			this.topPat = 0;
		}
					
    },
	
	_getSensImg: function() {
		
		var isWidth=true;
		
		if (this._oriPor(this.width,this.height)){
			if (this._oriPor(this.widthImg,this.heightImg)) {
				if (this._getRat(this.widthImg,this.heightImg,'por')<this._getRat(this.width,this.height,'por')) isWidth = false;
			} else { isWidth = false; }
		} else {
			if (!this._oriPor(this.widthImg,this.heightImg) && (this._getRat(this.widthImg,this.heightImg,'pay')>this._getRat(this.width,this.height,'pay'))) isWidth = false;
		}	
		
		if (isWidth) { 
			this.widthPat=this.widthImg;
			this.scalePat = this.width/this.widthImg;
			this.heightPat=Math.round(this.height/this.scalePat);						
		} else { 
			this.heightPat=this.heightImg;
			this.scalePat = this.height/this.heightImg;
			this.widthPat=Math.round(this.width/this.scalePat);			
		}
	...
	_oriPor : function (w,h){		
		if (w>h) return false;
		else return true;					
	},
	
	_getRat : function (w,h,ori){		
		if (ori=='pay') return (w/h);
		else return (h/w);			
	},
    ...
    _initElement: function(element) {
      ...
      fabric.util.addClass(this.getElement(), fabric.Imagepattern.CSS_CANVAS);
    },
  ...
  fabric.Imagepattern.CSS_CANVAS = 'canvas-img';

  fabric.Imagepattern.prototype.getSvgSrc = fabric.Imagepattern.prototype.getSrc;

  fabric.Imagepattern.ATTRIBUTE_NAMES =
   ...
   */
  fabric.Imagepattern.fromObject = function(object, callback) {
  
	fabric.util.loadImage(object.src, function(img) {
	
      callback && callback(new fabric.Imagepattern(img, object));check_nbe();
      
    }, null, object.crossOrigin);
  };
 
 
  fabric.Imagepattern.fromURL = function(url, callback, imgOptions) {
    fabric.util.loadImage(url, function(img) {
      callback && callback(new fabric.Imagepattern(img, imgOptions));check_nbe();
    }, null, imgOptions && imgOptions.crossOrigin);
  };
  
  fabric.Imagepattern.fromElement = function(element, callback, options) {
    var parsedAttributes = fabric.parseAttributes(element, fabric.Imagepattern.ATTRIBUTE_NAMES),
    ...
    fabric.Imagepattern.fromURL(parsedAttributes['xlink:href'], callback,
      extend((options ? fabric.util.object.clone(options) : { }), parsedAttributes));
  };
  /* _FROM_SVG_END_ */

  
  fabric.Imagepattern.async = true;

  fabric.Imagepattern.pngCompression = 1;
...
(function(global) {
   ...
   * @param {Object} [imgOptions] Options object
   */
  fabric.Image.fromPattern = function(url, callback, imgOptions) {
    fabric.util.loadImage(url, function(img) {
      callback && callback(new fabric.Image(img, imgOptions));
    }, null, imgOptions && imgOptions.crossOrigin);
  };
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @lends fabric.Text.prototype */ {
   ...
   //   this.clipTo && fabric.util.clipContext(this, ctx);
    ...
    _translateForTextAlign: function(ctx) {
      if (this.textAlign !== 'left' && this.textAlign !== 'justify' && this.CraArcPacrA == "plat") {
    ...
    _getTextHeight: function(ctx) {
	   ...
	   if (this.CraArcPacrA && this.CraArcPacrA != "plat") {
			return ctx.measureText(this._textLines[0]).width * this.CraARCPacrA;
		} else {
    ...
    _getTextWidth: function(ctx) {
		...
		if (this.CraArcPacrA && this.CraArcPacrA != "plat") {
			var maxWidth = this._getLineWidth(ctx, 0) * this.CraARCPacrA
		} else {
			var maxWidth = this._getLineWidth(ctx, 0);
		}
    ...
    _renderChars: function(method, ctx, chars, left, top) {
       if (this.CraArcPacrA && this.CraArcPacrA == "arc") {
                var dim;
                var angle = Math.PI * this.CraArCPacrA;
                var radius = ((ctx.measureText(chars.replace(/\s+/g, "")).width * this.CraARCPacrA) - (this.fontSize * this.lineHeight)) / 2;
                if (radius < 10) {
                    radius = 10
                }
                ...
                for (var n = 0; n < len; n++) {
                    if (n != 0) {
                        ctx.rotate(angle / (len - 1))
                    }
                    ctx.save();
                    ctx.translate(0, -1 * radius);
                    if (chars[n].toLowerCase() == "m") {
                        dim = (ctx.measureText(chars[n]).width / 2) * -1
                    } else {
                        dim = (ctx.measureText(chars[n]).width / 2) * -1
                    }
                    ctx.translate(dim, 0);
                    ctx[method](chars[n], left, top);
                    ctx.restore()
                }
                ctx.restore()
            } else {
                if (this.CraArcPacrA && this.CraArcPacrA == "arc_inv") {
                    var dim;
                    var angle = Math.PI * this.CraArCPacrA;
                    var radius = ((ctx.measureText(chars.replace(/\s+/g, "")).width * this.CraARCPacrA) - (this.fontSize * this.lineHeight)) / 2;
                    if (radius < 10) {
                    ...
                    for (var n = 0; n < len; n++) {
                        if (n != 0) {
                            ctx.rotate((angle / (len - 1)) * -1)
                        }
                        ctx.save();
                        ctx.translate(0, (-1 * radius) - (this.fontSize * this.lineHeight));
                        ctx.rotate(180 * Math.PI / 180);
                        if (chars[n].toLowerCase() == "m") {
                            dim = (ctx.measureText(chars[n]).width / 3) * -1
                        } else {
                            dim = (ctx.measureText(chars[n]).width / 2) * -1
                        }
                        ctx.translate(dim, 0);
    ...
    _renderTextFill: function(ctx) {
		...
		for (var i = 0, len = this._textLines.length; i < len; i++) {
			...
			CraPaXs += heightOfLine;

			 if (this.CraArcPacrA && this.CraArcPacrA != "plat") {
				this._renderTextLine("fillText", ctx, this._textLines[i], 0, 0, i)
			} else {
				this._renderTextLine("fillText", ctx, this._textLines[i], this._getLeftOffset(), this._getTopOffset()  + CraPaXs, i  );
			}
		}
      ...
      for (var i = 0, len = this._textLines.length; i < len; i++) {
        var heightOfLine = this._getHeightOfLine(ctx, i),
            maxHeight = heightOfLine / this.lineHeight;
				CraPaXs += heightOfLine;
                if (this.CraArcPacrA && this.CraArcPacrA != "plat") {
                    this._renderTextLine("strokeText", ctx, this._textLines[i], 0, 0, i)
                } else {
                    this._renderTextLine("strokeText", ctx, this._textLines[i], this._getLeftOffset() , this._getTopOffset()  + CraPaXs, i)
                }
...
var sav_colorpick;
var histo_color = [];
function add_histo(color){
	
	if (histo_color.indexOf(color)==-1){
		histo_color.push(color);					
	}
	if (histo_color.length>long_histo) histo_color.shift();
	$('#histo_palett').empty();
	jQuery.each(histo_color, function(i, val) {
		$('#histo_palett').append('<span class="sp-thumb-el sp-thumb-light" style="background-color:'+val+'" onclick="$(\'#flat\').spectrum(\'set\', \''+val+'\');change_colorpicker(\''+val+'\');"></span>'); 
	});				
...
var attribut_gen_svg = ["extractAttributes", "attr", "name", "value", "each", "required_attributes", "points", "d", "", "parseCoordinates", "split", "coords", "length", "push", "translate", "scale", "rotate", "skewX", "skewY", "matrix", "compareAttributes", " - ", " = ", " (not ", ")", "log", "write", "<", "path", " ", "=\"", "getPathString", "\"", "/>", "circle", "cx", "cy", "r", "PI", "sin", "cos", "ellipse", "rx", "ry", "rect", "x", "y", "width", "height", "line", "x1", "y1", "x2", "y2", "polyline", "join", "polygon", "commands", "mlhvqtcs", "MLHVQTCS", "MmLlqQtTcCsS", "MLQTCS", "z", "exec", "m", "M", "toAbsolute", "indexOf", "h", "v", "toUpperCase", "H", "V", "toRelative", "toLowerCase", "A", "a", "prototype", "nodeName", "attributes", "html", "#error-message", "absolute", "\x0A", "val", "#input-svg", "#input-transformation"];

function XMLElement() {
    this[attribut_gen_svg[0]] = function(_0x60d4x2) {
        this[attribut_gen_svg[1]] = {};
        var _0x60d4x3 = this;
        if (_0x60d4x2) {
            $[attribut_gen_svg[4]](_0x60d4x2, function(_0x60d4x4, _0x60d4x5) {
                _0x60d4x3[attribut_gen_svg[1]][_0x60d4x5[attribut_gen_svg[2]]] = _0x60d4x5[attribut_gen_svg[3]];
            });
        };
        for (var _0x60d4x6 in this[attribut_gen_svg[5]]) {
            var _0x60d4x5 = this[attribut_gen_svg[5]][_0x60d4x6];
            if (_0x60d4x5 === attribut_gen_svg[6] || _0x60d4x5 === attribut_gen_svg[7]) {
                if (!this[attribut_gen_svg[1]][_0x60d4x5]) {
                    this[attribut_gen_svg[1]][_0x60d4x5] = attribut_gen_svg[8];
                };
                this[attribut_gen_svg[9]](this[attribut_gen_svg[1]][_0x60d4x5]);
            } else {
                if (!this[attribut_gen_svg[1]][_0x60d4x5]) {
                    this[attribut_gen_svg[1]][_0x60d4x5] = 0;
                };
                this[attribut_gen_svg[1]][_0x60d4x5] = parseFloat(this[attribut_gen_svg[1]][_0x60d4x5]);
            };
        };
    };
    this[attribut_gen_svg[9]] = function(_0x60d4x7) {
        var _0x60d4x8 = _0x60d4x7[attribut_gen_svg[10]](/(?:\s*,\s*)|(?:\s+)/);
        this[attribut_gen_svg[11]] = [];
        for (var _0x60d4x4 = 0; _0x60d4x4 < _0x60d4x8[attribut_gen_svg[12]]; _0x60d4x4 += 2) {
            this[attribut_gen_svg[11]][attribut_gen_svg[13]]([parseFloat(_0x60d4x8[_0x60d4x4]), parseFloat(_0x60d4x8[_0x60d4x4 + 1])]);
        };
    };
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        return false;
    };
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        return false;
    };
    this[attribut_gen_svg[16]] = function(_0x60d4xd, _0x60d4xe, _0x60d4xf) {
        return false;
    };
    this[attribut_gen_svg[17]] = function(_0x60d4xd) {
        return false;
    };
    this[attribut_gen_svg[18]] = function(_0x60d4xd) {
        return false;
    };
    this[attribut_gen_svg[19]] = function(_0x60d4x6, _0x60d4x10, _0x60d4x11, _0x60d4x12, _0x60d4x13, _0x60d4x14) {
        if (_0x60d4x10 === 0 && _0x60d4x11 === 0) {
            this[attribut_gen_svg[15]](_0x60d4x6, _0x60d4x12);
            this[attribut_gen_svg[14]](_0x60d4x13, _0x60d4x14);
        } else {
            return false;
        };
    };
    this[attribut_gen_svg[20]] = function(_0x60d4x15) {
        for (var _0x60d4x4 in _0x60d4x15) {
            if (this[attribut_gen_svg[1]][_0x60d4x4] != _0x60d4x15[_0x60d4x4]) {
                console[attribut_gen_svg[25]](attribut_gen_svg[21] + _0x60d4x4 + attribut_gen_svg[22] + this[attribut_gen_svg[1]][_0x60d4x4] + attribut_gen_svg[23] + _0x60d4x15[_0x60d4x4] + attribut_gen_svg[24]);
                return false;
            };
        };
        return true;
    };
    this[attribut_gen_svg[26]] = function(_0x60d4x16) {
        if (_0x60d4x16 === undefined) {
            _0x60d4x16 = function(_0x60d4x17) {
                return _0x60d4x17;
            };
        };
        var _0x60d4x18 = attribut_gen_svg[27] + this[attribut_gen_svg[2]];
        for (var _0x60d4x6 in this[attribut_gen_svg[1]]) {
            if (this[attribut_gen_svg[2]] === attribut_gen_svg[28] && _0x60d4x6 === attribut_gen_svg[7]) {
                _0x60d4x18 += attribut_gen_svg[29] + _0x60d4x6 + attribut_gen_svg[30] + this[attribut_gen_svg[31]](_0x60d4x16) + attribut_gen_svg[32];
            } else {
                if (this[attribut_gen_svg[1]][_0x60d4x6] !== 0) {
                    _0x60d4x18 += attribut_gen_svg[29] + _0x60d4x6 + attribut_gen_svg[30] + _0x60d4x16(this[attribut_gen_svg[1]][_0x60d4x6]) + attribut_gen_svg[32];
                };
            };
        };
        _0x60d4x18 += attribut_gen_svg[33];
        return _0x60d4x18;
    };
};

function XML_Circle(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[34];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[35], attribut_gen_svg[36], attribut_gen_svg[37]];
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        this[attribut_gen_svg[1]][attribut_gen_svg[35]] += _0x60d4x9;
        this[attribut_gen_svg[1]][attribut_gen_svg[36]] += _0x60d4xa;
    };
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        if (_0x60d4xb === _0x60d4xc) {
            this[attribut_gen_svg[1]][attribut_gen_svg[35]] *= _0x60d4xb;
            this[attribut_gen_svg[1]][attribut_gen_svg[36]] *= _0x60d4xc;
            this[attribut_gen_svg[1]][attribut_gen_svg[37]] = _0x60d4xb >= 0 ? this[attribut_gen_svg[1]][attribut_gen_svg[37]] * _0x60d4xb : this[attribut_gen_svg[1]][attribut_gen_svg[37]] * -_0x60d4xb;
        } else {
            return false;
        };
    };
    this[attribut_gen_svg[16]] = function(_0x60d4x6, _0x60d4xe, _0x60d4xf) {
        var _0x60d4x1a = _0x60d4x6 * Math[attribut_gen_svg[38]] / 180;
        var _0x60d4x1b = Math[attribut_gen_svg[39]](_0x60d4x1a);
        var _0x60d4x1c = Math[attribut_gen_svg[40]](_0x60d4x1a);
        console[attribut_gen_svg[25]](_0x60d4x6, _0x60d4xe, _0x60d4xf);
        var _0x60d4x9 = this[attribut_gen_svg[1]][attribut_gen_svg[35]] - _0x60d4xe;
        var _0x60d4xa = this[attribut_gen_svg[1]][attribut_gen_svg[36]] - _0x60d4xf;
        this[attribut_gen_svg[1]][attribut_gen_svg[35]] = _0x60d4x1c * _0x60d4x9 - _0x60d4x1b * _0x60d4xa + _0x60d4xe;
        this[attribut_gen_svg[1]][attribut_gen_svg[36]] = _0x60d4x1b * _0x60d4x9 + _0x60d4x1c * _0x60d4xa + _0x60d4xf;
    };
};

function XML_Ellipse(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[41];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[35], attribut_gen_svg[36], attribut_gen_svg[42], attribut_gen_svg[43]];
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        this[attribut_gen_svg[1]][attribut_gen_svg[35]] += _0x60d4x9;
        this[attribut_gen_svg[1]][attribut_gen_svg[36]] += _0x60d4xa;
    ...
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        ...
        this[attribut_gen_svg[1]][attribut_gen_svg[36]] *= _0x60d4xc;
        if (_0x60d4xb >= 0) {
            this[attribut_gen_svg[1]][attribut_gen_svg[42]] *= _0x60d4xb;
        } else {
            this[attribut_gen_svg[1]][attribut_gen_svg[42]] *= -_0x60d4xb;
        };
        if (_0x60d4xc >= 0) {
            this[attribut_gen_svg[1]][attribut_gen_svg[43]] *= _0x60d4xc;
        } else {
            this[attribut_gen_svg[1]][attribut_gen_svg[43]] *= -_0x60d4xc;
        };
    };
...
function XML_Rect(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[44];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[45], attribut_gen_svg[46], attribut_gen_svg[47], attribut_gen_svg[48]];
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        this[attribut_gen_svg[1]][attribut_gen_svg[45]] += _0x60d4x9;
        this[attribut_gen_svg[1]][attribut_gen_svg[46]] += _0x60d4xa;
    };
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        this[attribut_gen_svg[1]][attribut_gen_svg[45]] *= _0x60d4xb;
        this[attribut_gen_svg[1]][attribut_gen_svg[46]] *= _0x60d4xc;
        if (_0x60d4xb >= 0) {
            this[attribut_gen_svg[1]][attribut_gen_svg[47]] *= _0x60d4xb;
        } else {
            this[attribut_gen_svg[1]][attribut_gen_svg[47]] *= -_0x60d4xb;
            this[attribut_gen_svg[1]][attribut_gen_svg[45]] -= this[attribut_gen_svg[1]][attribut_gen_svg[47]];
        };
        if (_0x60d4xc >= 0) {
            this[attribut_gen_svg[1]][attribut_gen_svg[48]] *= _0x60d4xc;
        } else {
            this[attribut_gen_svg[1]][attribut_gen_svg[48]] *= -_0x60d4xc;
            this[attribut_gen_svg[1]][attribut_gen_svg[46]] -= this[attribut_gen_svg[1]][attribut_gen_svg[48]];
        };
    };
};

function XML_Line(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[49];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[50], attribut_gen_svg[51], attribut_gen_svg[52], attribut_gen_svg[53]];
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        this[attribut_gen_svg[1]][attribut_gen_svg[50]] += _0x60d4x9;
        this[attribut_gen_svg[1]][attribut_gen_svg[52]] += _0x60d4x9;
        this[attribut_gen_svg[1]][attribut_gen_svg[51]] += _0x60d4xa;
        this[attribut_gen_svg[1]][attribut_gen_svg[53]] += _0x60d4xa;
    };
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        this[attribut_gen_svg[1]][attribut_gen_svg[50]] *= _0x60d4xb;
        this[attribut_gen_svg[1]][attribut_gen_svg[52]] *= _0x60d4xb;
        this[attribut_gen_svg[1]][attribut_gen_svg[51]] *= _0x60d4xc;
        this[attribut_gen_svg[1]][attribut_gen_svg[53]] *= _0x60d4xc;
    };
    this[attribut_gen_svg[19]] = function(_0x60d4x6, _0x60d4x10, _0x60d4x11, _0x60d4x12, _0x60d4x13, _0x60d4x14) {
        var _0x60d4x17 = _0x60d4x6 * this[attribut_gen_svg[1]][attribut_gen_svg[50]] + _0x60d4x11 * this[attribut_gen_svg[1]][attribut_gen_svg[51]] + _0x60d4x13;
        var _0x60d4x20 = _0x60d4x10 * this[attribut_gen_svg[1]][attribut_gen_svg[50]] + _0x60d4x12 * this[attribut_gen_svg[1]][attribut_gen_svg[51]] + _0x60d4x14;
        this[attribut_gen_svg[1]][attribut_gen_svg[50]] = _0x60d4x17;
        this[attribut_gen_svg[1]][attribut_gen_svg[51]] = _0x60d4x20;
        var _0x60d4x17 = _0x60d4x6 * this[attribut_gen_svg[1]][attribut_gen_svg[52]] + _0x60d4x11 * this[attribut_gen_svg[1]][attribut_gen_svg[53]] + _0x60d4x13;
        var _0x60d4x20 = _0x60d4x10 * this[attribut_gen_svg[1]][attribut_gen_svg[52]] + _0x60d4x12 * this[attribut_gen_svg[1]][attribut_gen_svg[53]] + _0x60d4x14;
        this[attribut_gen_svg[1]][attribut_gen_svg[52]] = _0x60d4x17;
        this[attribut_gen_svg[1]][attribut_gen_svg[53]] = _0x60d4x20;
    };
};

function XML_Polyline(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[54];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[6]];
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        for (var _0x60d4x4 in this[attribut_gen_svg[11]]) {
            this[attribut_gen_svg[11]][_0x60d4x4][0] += _0x60d4x9;
            this[attribut_gen_svg[11]][_0x60d4x4][1] += _0x60d4xa;
        };
        this[attribut_gen_svg[1]][attribut_gen_svg[6]] = this[attribut_gen_svg[11]][attribut_gen_svg[55]](attribut_gen_svg[29]);
    };
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        for (var _0x60d4x4 in this[attribut_gen_svg[11]]) {
            this[attribut_gen_svg[11]][_0x60d4x4][0] *= _0x60d4xb;
            this[attribut_gen_svg[11]][_0x60d4x4][1] *= _0x60d4xc;
        };
        this[attribut_gen_svg[1]][attribut_gen_svg[6]] = this[attribut_gen_svg[11]][attribut_gen_svg[55]](attribut_gen_svg[29]);
    };
    this[attribut_gen_svg[19]] = function(_0x60d4x6, _0x60d4x10, _0x60d4x11, _0x60d4x12, _0x60d4x13, _0x60d4x14) {
        for (var _0x60d4x4 in this[attribut_gen_svg[11]]) {
            var _0x60d4x17 = _0x60d4x6 * this[attribut_gen_svg[11]][_0x60d4x4][0] + _0x60d4x11 * this[attribut_gen_svg[11]][_0x60d4x4][1] + _0x60d4x13;
            var _0x60d4x20 = _0x60d4x10 * this[attribut_gen_svg[11]][_0x60d4x4][0] + _0x60d4x12 * this[attribut_gen_svg[11]][_0x60d4x4][1] + _0x60d4x14;
            this[attribut_gen_svg[11]][_0x60d4x4][0] = _0x60d4x17;
            this[attribut_gen_svg[11]][_0x60d4x4][1] = _0x60d4x20;
        };
        this[attribut_gen_svg[1]][attribut_gen_svg[6]] = this[attribut_gen_svg[11]][attribut_gen_svg[55]](attribut_gen_svg[29]);
    };
};

function XML_Polygon(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[56];
    this[attribut_gen_svg[0]](_0x60d4x2);
};

function XML_Path(_0x60d4x2) {
    this[attribut_gen_svg[2]] = attribut_gen_svg[28];
    this[attribut_gen_svg[5]] = [attribut_gen_svg[7]];
    this[attribut_gen_svg[57]] = [];
    var _0x60d4x24 = {
        "m": 2,
        "l": 2,
        "c": 6,
    ...
    var _0x60d4x25 = attribut_gen_svg[58];
    var _0x60d4x26 = attribut_gen_svg[59];
    var _0x60d4x27 = attribut_gen_svg[60];
    var _0x60d4x28 = attribut_gen_svg[61];
    var _0x60d4x29 = attribut_gen_svg[58];
    this[attribut_gen_svg[9]] = function(_0x60d4x2a) {
        var _0x60d4x2b = /([ACHLMQSTV])([-\+\d\.\s,e]*)(z)?/gi;
        while (commands = _0x60d4x2b[attribut_gen_svg[63]](_0x60d4x2a)) {
            this[attribut_gen_svg[57]][attribut_gen_svg[13]]([commands[1], extractDigits(commands[2]), commands[3] ? attribut_gen_svg[62] : attribut_gen_svg[8]]);
        };
        if (this[attribut_gen_svg[57]][0][0] === attribut_gen_svg[64]) {
            this[attribut_gen_svg[57]][0][0] = attribut_gen_svg[65];
        };
    };
    this[attribut_gen_svg[66]] = function() {
        var _0x60d4x2c = 0;
        var _0x60d4x2d = 0;
        for (var _0x60d4x4 in this[attribut_gen_svg[57]]) {
            var _0x60d4x2e = this[attribut_gen_svg[57]][_0x60d4x4][0];
            var _0x60d4x2f = this[attribut_gen_svg[57]][_0x60d4x4][1];
            if (_0x60d4x25[attribut_gen_svg[67]](_0x60d4x2e) > -1 && _0x60d4x24[_0x60d4x2e]) {
                var _0x60d4x30 = _0x60d4x24[_0x60d4x2e];
                if (_0x60d4x2e === attribut_gen_svg[68]) {
                    _0x60d4x2f[0] += _0x60d4x2c;
                    _0x60d4x2c = _0x60d4x2f;
                } else {
                    if (_0x60d4x2e === attribut_gen_svg[69]) {
                        _0x60d4x2f[0] += _0x60d4x2d;
                        _0x60d4x2d = _0x60d4x2f;
                    } else {
                        for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 2) {
                            _0x60d4x2f[_0x60d4x31] += _0x60d4x2c;
                            _0x60d4x2f[_0x60d4x31 + 1] += _0x60d4x2d;
                            if (_0x60d4x31 % _0x60d4x30 === _0x60d4x30 - 2) {
                                _0x60d4x2c = _0x60d4x2f[_0x60d4x31];
                                _0x60d4x2d = _0x60d4x2f[_0x60d4x31 + 1];
                            };
                        };
                    };
                };
                this[attribut_gen_svg[57]][_0x60d4x4][0] = _0x60d4x2e[attribut_gen_svg[70]]();
            } else {
                var _0x60d4x30 = _0x60d4x2f[attribut_gen_svg[12]];
                if (_0x60d4x2e === attribut_gen_svg[71]) {
                    _0x60d4x2c = _0x60d4x2f[_0x60d4x30 - 1];
                } else {
                    if (_0x60d4x2e === attribut_gen_svg[72]) {
                        _0x60d4x2d = _0x60d4x2f[_0x60d4x30 - 1];
                    } else {
                        _0x60d4x2c = _0x60d4x2f[_0x60d4x30 - 2];
                        _0x60d4x2d = _0x60d4x2f[_0x60d4x30 - 1];
                    };
                };
            };
    ...
    this[attribut_gen_svg[73]] = function() {
        var _0x60d4x2c = 0;
        var _0x60d4x2d = 0;
        for (var _0x60d4x4 in this[attribut_gen_svg[57]]) {
            var _0x60d4x2e = this[attribut_gen_svg[57]][_0x60d4x4][0];
            var _0x60d4x2f = this[attribut_gen_svg[57]][_0x60d4x4][1];
            if (_0x60d4x26[attribut_gen_svg[67]](_0x60d4x2e) > -1 && _0x60d4x24[_0x60d4x2e]) {
                var _0x60d4x30 = _0x60d4x24[_0x60d4x2e];
                if (_0x60d4x2e === attribut_gen_svg[71]) {
                    var _0x60d4x32 = _0x60d4x2f[0];
                    _0x60d4x2f[0] -= _0x60d4x2c;
                    _0x60d4x2c = _0x60d4x32;
                } else {
                    if (_0x60d4x2e === attribut_gen_svg[72]) {
                        var _0x60d4x33 = _0x60d4x2f[0];
                        _0x60d4x2f[0] -= _0x60d4x2d;
                        _0x60d4x2d = _0x60d4x33;
                    } else {
                        for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 2) {
                            var _0x60d4x32 = _0x60d4x2f[_0x60d4x31];
                            var _0x60d4x33 = _0x60d4x2f[_0x60d4x31 + 1];
                            _0x60d4x2f[_0x60d4x31] -= _0x60d4x2c;
                            _0x60d4x2f[_0x60d4x31 + 1] -= _0x60d4x2d;
                            if (_0x60d4x31 % _0x60d4x30 === _0x60d4x30 - 2) {
                                _0x60d4x2c = _0x60d4x32;
                ...
                this[attribut_gen_svg[57]][_0x60d4x4][0] = _0x60d4x2e[attribut_gen_svg[74]]();
            } else {
                var _0x60d4x30 = _0x60d4x2f[attribut_gen_svg[12]];
                if (_0x60d4x2e === attribut_gen_svg[68]) {
                    _0x60d4x2c += _0x60d4x2f[_0x60d4x30 - 1];
    ...
    this[attribut_gen_svg[31]] = function(_0x60d4x16) {
        var _0x60d4x34 = attribut_gen_svg[8];
        for (var _0x60d4x4 in this[attribut_gen_svg[57]]) {
            var _0x60d4x35 = this[attribut_gen_svg[57]][_0x60d4x4];
            _0x60d4x34 += _0x60d4x35[0];
            for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x35[1][attribut_gen_svg[12]]; _0x60d4x31++) {
                if (_0x60d4x31 > 0) {
                    _0x60d4x34 += attribut_gen_svg[29];
                };
                _0x60d4x34 += _0x60d4x16(_0x60d4x35[1][_0x60d4x31]);
            };
            _0x60d4x34 += _0x60d4x35[2];
        };
        return _0x60d4x34;
    };
    this[attribut_gen_svg[0]](_0x60d4x2);
    this[attribut_gen_svg[14]] = function(_0x60d4x9, _0x60d4xa) {
        for (var _0x60d4x4 in this[attribut_gen_svg[57]]) {
            var _0x60d4x2e = this[attribut_gen_svg[57]][_0x60d4x4][0];
            var _0x60d4x2f = this[attribut_gen_svg[57]][_0x60d4x4][1];
            if (_0x60d4x28[attribut_gen_svg[67]](_0x60d4x2e) > -1) {
                for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 2) {
                    _0x60d4x2f[_0x60d4x31] += _0x60d4x9;
                    _0x60d4x2f[_0x60d4x31 + 1] += _0x60d4xa;
                };
            } else {
                if (_0x60d4x2e === attribut_gen_svg[71]) {
                    for (var _0x60d4x31 in _0x60d4x2f) {
                        _0x60d4x2f[_0x60d4x31] += _0x60d4x9;
                    };
                } else {
                    if (_0x60d4x2e === attribut_gen_svg[72]) {
                        for (var _0x60d4x31 in _0x60d4x2f) {
                            _0x60d4x2f[_0x60d4x31] += _0x60d4xa;
                        };
                    } else {
                        if (_0x60d4x2e === attribut_gen_svg[75]) {
                            for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 7) {
                                _0x60d4x2f[_0x60d4x31 + 5] += _0x60d4x9;
                                _0x60d4x2f[_0x60d4x31 + 6] += _0x60d4xa;
                            };
                        } else {
                            if (_0x60d4x29[attribut_gen_svg[67]](_0x60d4x2e) === -1) {
                                return false;
                            };
                        };
                    };
    ...
    this[attribut_gen_svg[15]] = function(_0x60d4xb, _0x60d4xc) {
        for (var _0x60d4x4 in this[attribut_gen_svg[57]]) {
            var _0x60d4x2e = this[attribut_gen_svg[57]][_0x60d4x4][0];
            var _0x60d4x2f = this[attribut_gen_svg[57]][_0x60d4x4][1];
            if (_0x60d4x27[attribut_gen_svg[67]](_0x60d4x2e) > -1) {
                for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 2) {
                    _0x60d4x2f[_0x60d4x31] *= _0x60d4xb;
                    _0x60d4x2f[_0x60d4x31 + 1] *= _0x60d4xc;
                };
            } else {
                if (_0x60d4x2e === attribut_gen_svg[75] || _0x60d4x2e === attribut_gen_svg[76]) {
                    return false;
                    for (var _0x60d4x31 = 0; _0x60d4x31 < _0x60d4x2f[attribut_gen_svg[12]]; _0x60d4x31 += 7) {
                        if (_0x60d4xb > 0) {
                            _0x60d4x2f[_0x60d4x31] *= _0x60d4xb;
                            ...
                            _0x60d4x2f[_0x60d4x31] *= -_0x60d4xb;
                            _0x60d4x2f[_0x60d4x31 + 4] = 1 - _0x60d4x2f[_0x60d4x31 + 4];
                        };
                        if (_0x60d4xc > 0) {
                            _0x60d4x2f[_0x60d4x31 + 1] *= _0x60d4xc;
                        } else {
                        ...
                        _0x60d4x2f[_0x60d4x31 + 5] *= _0x60d4xb;
                        _0x60d4x2f[_0x60d4x31 + 6] *= _0x60d4xc;
                    };
                } else {
                    return false;
                };
...
for (var shape in XML_shapes) {
    XML_shapes[shape][attribut_gen_svg[77]] = new XMLElement();
};
XML_Polygon[attribut_gen_svg[77]] = new XML_Polyline;
var parseX = function(_0x60d4x38) {
    var _0x60d4x39 = [];
    $[attribut_gen_svg[4]]($(_0x60d4x38), function(_0x60d4x4, _0x60d4x3a) {
        var _0x60d4x3b = this[attribut_gen_svg[78]][attribut_gen_svg[74]]();
        if (_0x60d4x3b in XML_shapes) {
            var _0x60d4x3a = new XML_shapes[_0x60d4x3b](this[attribut_gen_svg[79]]);
            _0x60d4x39[attribut_gen_svg[13]](_0x60d4x3a);
        };
    });
    return _0x60d4x39;
};
var extractDigits = function(_0x60d4x3d) {
    if (!_0x60d4x3d) {
        return;
    };
    var _0x60d4x3e = /([-+]?[\d\.]+)([eE][-+]?[\d\.]+)?/g;
    var _0x60d4x3f = [];
    while (digit = _0x60d4x3e[attribut_gen_svg[63]](_0x60d4x3d)) {
        _0x60d4x3f[attribut_gen_svg[13]](parseFloat(digit));
    };
    return _0x60d4x3f;
};
var parseTransformString = function(_0x60d4x41) {
    var _0x60d4x3e = /([-+]?\d*\.?\d+)/g;
    var _0x60d4x42 = /translate\s*\(([-\+\d\.\s,e]+)\)/i;
    var _0x60d4x43 = /scale\s*\(([-\+\d\.\s,e]+)\)/i;
    var _0x60d4x44 = /rotate\s*\(([-\+\d\.\s,e]+)\)/i;
    var _0x60d4x45 = /matrix\s*\(([-\+\d\.\s,e]+)\)/i;
    if (translate_digits = _0x60d4x42[attribut_gen_svg[63]](_0x60d4x41)) {
        var _0x60d4x3f = extractDigits(translate_digits[1]);
        if (_0x60d4x3f[attribut_gen_svg[12]] > 0) {
            return function(_0x60d4x3a) {
                return _0x60d4x3a[attribut_gen_svg[14]](_0x60d4x3f[0], _0x60d4x3f[1] ? _0x60d4x3f[1] : 0);
            };
        };
    };
    if (scale_digits = _0x60d4x43[attribut_gen_svg[63]](_0x60d4x41)) {
        var _0x60d4x3f = extractDigits(scale_digits[1]);
        if (_0x60d4x3f[attribut_gen_svg[12]] > 0) {
            return function(_0x60d4x3a) {
                return _0x60d4x3a[attribut_gen_svg[15]](_0x60d4x3f[0], _0x60d4x3f[1] ? _0x60d4x3f[1] : _0x60d4x3f[0]);
            };
        };
    };
    if (matrix_digits = _0x60d4x45[attribut_gen_svg[63]](_0x60d4x41)) {
        var _0x60d4x3f = extractDigits(matrix_digits[1]);
        if (_0x60d4x3f[attribut_gen_svg[12]] === 6) {
            return function(_0x60d4x3a) {
                return _0x60d4x3a[attribut_gen_svg[19]](_0x60d4x3f[0], _0x60d4x3f[1], _0x60d4x3f[2], _0x60d4x3f[3], _0x60d4x3f[4], _0x60d4x3f[5]);
            };
        };
    };
    if (rotate_digits = _0x60d4x44[attribut_gen_svg[63]](_0x60d4x41)) {
        var _0x60d4x3f = extractDigits(rotate_digits[1]);
        if (_0x60d4x3f[attribut_gen_svg[12]] > 0) {
            return function(_0x60d4x3a) {
                return _0x60d4x3a[attribut_gen_svg[16]](_0x60d4x3f[0], _0x60d4x3f[1] ? _0x60d4x3f[1] : 0, _0x60d4x3f[2] ? _0x60d4x3f[2] : 0);
            };
        };
    };
};
var getDecimalPlaceFunction = function() {
    return function(_0x60d4x17) {
        return _0x60d4x17;
    };
};

function get_substring(_0x60d4x48, _0x60d4x49) {
    $(attribut_gen_svg[81])[attribut_gen_svg[80]](attribut_gen_svg[8]);
    var _0x60d4x39 = parseX(_0x60d4x48);
    if (_0x60d4x39[attribut_gen_svg[12]] === 0) {
        return;
    };
    var _0x60d4x4a = attribut_gen_svg[8];
    var _0x60d4x16 = getDecimalPlaceFunction();
    var _0x60d4x41 = _0x60d4x49;
    var _0x60d4x4b = false;
    if (_0x60d4x41 !== attribut_gen_svg[8]) {
        _0x60d4x4b = parseTransformString(_0x60d4x41);
    };
    var _0x60d4x4c = attribut_gen_svg[82];
    var _0x60d4x4d = attribut_gen_svg[66];
    for (e in _0x60d4x39) {
        var _0x60d4x3a = _0x60d4x39[e];
        if (_0x60d4x4b && _0x60d4x4b(_0x60d4x3a) === false) {};
        if (_0x60d4x3a[attribut_gen_svg[2]] === attribut_gen_svg[28] && _0x60d4x4d) {
            _0x60d4x3a[_0x60d4x4d]();
        };
        _0x60d4x4a += _0x60d4x3a[attribut_gen_svg[26]](_0x60d4x16) + attribut_gen_svg[83];
    };
    return _0x60d4x4a;
};
var addExampleElement = function(_0x60d4x3b) {
    var _0x60d4x3a = XML_elements[_0x60d4x3b];
    $(attribut_gen_svg[85])[attribut_gen_svg[84]](_0x60d4x3a);
};
var addExampleTransformation = function(_0x60d4x3b) {
    var _0x60d4x4b = test_transformations[_0x60d4x3b];
    $(attribut_gen_svg[86])[attribut_gen_svg[84]](_0x60d4x4b);
};